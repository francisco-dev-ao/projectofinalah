...
const AdminInvoiceDetailsHeader: React.FC<AdminInvoiceDetailsHeaderProps> = ({
  invoice,
  isGeneratingPDF,
  isSharing,
  isPrinting = false,
  ...
  return (
    <div className="space-y-4">
      <Button asChild variant="ghost" size="sm">
        <Link to="/admin/invoices" className="flex items-center">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start gap-4">
        <div className="space-y-2">
          ...
          <div className="text-sm text-gray-600 space-y-1">
            ...
            {invoice?.orders?.profiles?.name && (
              <p><span className="font-medium">Cliente:</span> {invoice.orders.profiles.name}</p>
            )}
          </div>
        </div>
...
export default AdminInvoiceDetailsHeader;
